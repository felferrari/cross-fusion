...
class DataLoader(Sequence):
    def __init__(self, batch_size, data_path, label_path, patch_size,
                 opt_bands, sar_bands, num_classes, shuffle = False, limit = None, tile = None):
        ...
        if self.tile is not None:
            self.used_files = [name  for name in self.list_files if int(name[:2])==self.tile]
    ...
    def get_labels(self, to_categorical = True):
        ...
        for i, f in enumerate(self.used_files):
            Y = np.load(os.path.join(self.label_path, f'label_{f}.npy'))
            if to_categorical:
                Y_label[i, :, :, :] = tf.keras.utils.to_categorical(Y, num_classes=self.num_classes)
            else:
    ...
    def get_inputs(self, tile):
        tile_files = [name  for name in self.used_files if int(name[:2])==tile]
        X_opt = np.empty((len(tile_files), self.patch_size, self.patch_size, self.opt_bands))
        X_sar = np.empty((len(tile_files), self.patch_size, self.patch_size, self.sar_bands))
        for i, f in enumerate(tile_files):
            X_o = np.load(os.path.join(self.data_path, f'opt_{f}.npy'))
            X_s = np.load(os.path.join(self.data_path, f'sar_{f}.npy'))
            X_opt[i, :, :, :] = X_o
            X_sar[i, :, :, :] = X_s
        return (X_opt, X_sar)

class DataLoader2(Sequence):
    def __init__(self, batch_size, data_path, label_path, patch_size,
                 opt_bands, sar_bands, num_classes, shuffle = False, limit = None, tile = None):
        self.data_path = data_path
        self.label_path = label_path
    ...
    def __getitem__(self, idx):
        ...
        return (tf.convert_to_tensor(X_opt, dtype=tf.float32),
            tf.convert_to_tensor(Y_label, dtype=tf.float32)
            )
